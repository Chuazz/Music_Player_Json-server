...
(async () => {
    const rawResponse = await fetch(myVar.songAPI);
    const songList = await rawResponse.json();
    ...
    const myApp = {
        ...
        renderConfig: function () {
            var songItems = myVar.$$(".song__item-body");
            ...
            if (likedSongId.length > 0) {
                likedSongId.forEach(songId => {
                    songItems.forEach(song => {
                        if(Number(song.dataset.songid) == songId){
                            song.querySelector(".fa-heart").classList.add("active");
                        }
                    })
                });
            }
        ...
        handleEvents: function () {
            ...
            myVar.playlist.addEventListener("click", (e) => {
                ...
                if (song && !downloadBtn && !likeSong) {
                    ...
                    var index = Number(parent.dataset.songid);
